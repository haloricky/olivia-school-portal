...
const CHAPTERS = [
  {
    ...
    fact: "Egypt is about 6\xD7 bigger than the UK \u2014 and 90% of it is pure desert!"
  },
  {
    id: "nile",
...
function MapScene() {
  return <div className="relative w-full rounded-2xl overflow-hidden" style={{ background: "#B3D9F2", height: 300 }}>
      <svg viewBox="0 0 1000 500" className="w-full" style={{ height: 300, objectFit: "cover" }}>
...
function ImgCard({ src, alt, caption, height = 300 }) {
  ...
  return <div className="relative w-full rounded-2xl overflow-hidden" style={{ height }}>
...
function ChapterScene({ chapter }) {
  ...
  if (chapter.kind === "image") {
    return <ImgCard src={chapter.img} alt={chapter.imgAlt} caption={chapter.imgCaption} />;
  }
  if (chapter.kind === "double") {
    return <DoubleImageScene
      img={chapter.img}
      imgAlt={chapter.imgAlt}
      imgCaption={chapter.imgCaption}
      img2={chapter.img2}
      img2Alt={chapter.img2Alt}
      img2Caption={chapter.img2Caption}
    />;
  }
  return null;
}
function EgyptStory() {
  ...
  return <div className="flex flex-col gap-3" style={{ fontFamily: "Nunito, sans-serif" }}>
      <div className="text-center">
        <h3 className="text-lg font-extrabold mb-0.5" style={{ color: "#b8860b" }}>
    ...
    style={{
      width: i === page ? 24 : 10,
      height: 10,
      backgroundColor: i === page ? "#daa520" : i < page ? "#90EE90" : "#e5e7eb"
    }}
  />)}
      </div>
  ...
  >
    ...
    disabled={page === 0}
    className="flex items-center gap-1.5 px-5 py-2.5 rounded-2xl font-extrabold text-sm transition-all active:translate-y-0.5 disabled:opacity-30"
