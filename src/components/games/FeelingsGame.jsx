...
const FEELINGS = [
  { id: 'happy', en: 'Happy', id_lang: 'Senang', emoji: '😄', color: '#FFD700', dark: '#c9a700', bg: '#FFFDE7',
  ...
  { id: 'sad', en: 'Sad', id_lang: 'Sedih', emoji: '😢', color: '#87CEEB', dark: '#4a9aba', bg: '#E3F2FD',
  ...
  { id: 'angry', en: 'Angry', id_lang: 'Marah', emoji: '😡', color: '#FF7043', dark: '#c0391a', bg: '#FBE9E7',
    description: "When something feels very unfair or frustrating and your body feels hot and tight. Everyone feels angry sometimes — it's normal!",
    helps: "Take 5 big deep breaths. Count slowly to 10. Go for a walk or jump around. Then talk about why you felt angry.",
    examples: "When something feels unfair, when your toy is broken, when someone is unkind." },
  { id: 'scared', en: 'Scared', id_lang: 'Takut', emoji: '😨', color: '#9C27B0', dark: '#6a0f7a', bg: '#F3E5F5',
  ...
  { id: 'proud', en: 'Proud', id_lang: 'Bangga', emoji: '😤', color: '#FF85A1', dark: '#cc5a74', bg: '#FFF0F5',
    ...
    helps: "Celebrate! Tell someone what you achieved. Do a happy dance. Remember this feeling for next time something is hard.",
    examples: "Finishing a puzzle, learning to read a word, helping someone, drawing a great picture!" },
  { id: 'surprised', en: 'Surprised', id_lang: 'Terkejut', emoji: '😲', color: '#FF9800', dark: '#c46a00', bg: '#FFF8E1',
    ...
    helps: "Take a breath, then figure out if the surprise was good or not. If it was good — enjoy it!",
  ...
  return (
    <div className="flex flex-col gap-4 font-display">
      ...
      <div className="grid grid-cols-3 gap-3">
        {FEELINGS.map(f => (
          <button key={f.id} onClick={() => setSelected(s => s === f.id ? null : f.id)}
            className="flex flex-col items-center gap-1 rounded-2xl py-3 px-2 transition-all active:scale-95"
            style={{ backgroundColor: selected === f.id ? f.bg : '#fafafa',
      ...
      {feeling && (
        <div className="rounded-2xl p-4 flex flex-col gap-3" style={{ backgroundColor: feeling.bg, border: `2.5px solid ${feeling.color}` }}>
      ...
      {!feeling && (
        <div className="rounded-2xl px-4 py-4 text-center" style={{ backgroundColor: '#FFF9E6', border: '2px solid #FFD700' }}>
