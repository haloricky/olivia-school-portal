...
export default function WorldMapGame() {
  ...
  if (won) {
    return (
      <>
        <Confetti />
        <div className="flex flex-col items-center gap-5 text-center p-6 min-h-64 justify-center">
          <div className="text-7xl animate-bounce-slow">🎉</div>
          <h2 className="font-display text-3xl text-primary">You know all 7 continents, Olivia!</h2>
          <div className="grid grid-cols-2 gap-2 mt-1 w-full max-w-md">
            {WORLD_CONTINENTS.map(c => (
              <div
                key={c.name}
                className="flex items-start gap-2 rounded-chunky px-3 py-2 text-sm text-left"
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="text-center">
        <h3 className="font-display text-xl text-primary mb-0.5">Find Each Continent on the Map!</h3>
        <p className="text-sm text-gray-500 font-bold">
          {selected
            ? <>Click <strong className="text-primary">{selected}</strong> on the map below 👇</>
            : 'Click a continent name to select it, then tap its spot on the map'}
        </p>
        <div className="flex justify-center gap-1.5 mt-2">
          {WORLD_CONTINENTS.map((_, i) => (
            <div
              key={i}
      ...
      <div className="relative w-full rounded-chunky overflow-hidden border-2 border-blue-200 shadow-md">
        <svg viewBox="0 0 1000 500" className="w-full block">
          ...
          {WORLD_CONTINENTS.map(c => {
            ...
            return (
              <g key={c.name} onClick={() => handleContinentClick(c.name)} style={{ cursor: selected && !isPlaced ? 'pointer' : 'default' }}>
                <path
                  d={c.path}
                ...
                />
                {isPlaced && (
                  <text x={c.labelX} y={c.labelY - 8} textAnchor="middle" fill="white"
                    fontSize={c.name === 'Antarctica' ? '11' : '13'} fontWeight="800"
                    fontFamily="Nunito, sans-serif" style={{ pointerEvents: 'none' }}>
                    {c.emoji} {c.name}
                  </text>
                )}
                {isWrong && (
                  <text x={c.labelX} y={c.labelY} textAnchor="middle" fill="#cc0000"
                    fontSize="18" fontFamily="Nunito, sans-serif" fontWeight="bold"
                    style={{ pointerEvents: 'none' }}>✗</text>
                )}
                {!isPlaced && !isWrong && selected && (
                  <text x={c.labelX} y={c.labelY + 4} textAnchor="middle" fill="#6b7280"
        ...
        {availableLabels.map(name => {
          ...
          return (
            <button
              ...
              className="press-btn flex items-center gap-1.5 px-3 py-2 rounded-chunky font-display text-sm border-2 transition-all"
              style={{
                backgroundColor: isSelected ? c.color : c.color + '22',
                borderColor: c.color,
                color: isSelected ? 'white' : '#333',
                boxShadow: isSelected ? `0 3px 0 ${c.color}aa` : undefined,
      ...
      {availableLabels.length === 0 && !won && (
        <p className="text-center text-green-500 font-display">Almost there…</p>
      )}

      <button onClick={handleReset} className="text-gray-400 text-xs font-bold underline self-center mt-1">
        Reset
      </button>
    </div>
  )
