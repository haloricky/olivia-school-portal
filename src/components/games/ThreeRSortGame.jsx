...
const CATEGORIES = [
  { key: "reduce",  label: "Reduce",  emoji: "📉", color: "#1D9E75", dark: "#116047", bg: "#E8FBF5", desc: "Use LESS — buy less, use less energy, create less waste" },
  ...
  { key: "recycle", label: "Recycle", emoji: "🗂️",  color: "#534AB7", dark: "#2f2880", bg: "#F0EEFF", desc: "Turn old materials INTO new things — paper, glass, plastic, metal" },
];

const ITEMS = [
  { label: "Turn off lights when you leave a room", emoji: "💡", category: "reduce", reason: "Using less electricity means fewer fossil fuels are burned. One small habit, multiplied by millions of people, makes a huge difference!" },
  { label: "Refill your water bottle instead of buying a new one", emoji: "🍶", category: "reuse", reason: "A reusable water bottle can replace up to 500 single-use plastic bottles per year! That's a lot of plastic kept out of the ocean." },
  { label: "Put paper in the recycling bin instead of the rubbish", emoji: "📰", category: "recycle", reason: "Recycling one tonne of paper saves 17 trees! Paper can be recycled up to 7 times before the fibres become too short to reuse." },
  { label: "Walk or cycle to nearby places instead of driving", emoji: "🚲", category: "reduce", reason: "Cars produce CO₂ that heats up our planet. Walking and cycling create zero emissions — and they're great exercise too!" },
  { label: "Use your old t-shirt as a cleaning rag", emoji: "👕", category: "reuse", reason: "Before throwing away worn-out clothes, think: can it become something else? Old t-shirts make fantastic cleaning cloths!" },
  { label: "Put glass jars in the recycling bin", emoji: "🫙", category: "recycle", reason: "Glass can be recycled endlessly without losing quality! Recycling glass saves up to 30% of the energy needed to make new glass." },
  { label: "Take a shorter shower to use less water", emoji: "🚿", category: "reduce", reason: "A 2-minute shorter shower saves 10 litres of water! Over a year, that's 3,650 litres saved — enough to fill a small swimming pool." },
  { label: "Use a cloth shopping bag instead of plastic bags", emoji: "👜", category: "reuse", reason: "One reusable cloth bag can replace over 1,000 plastic bags in its lifetime. Plastic bags can take 1,000 years to decompose!" },
  { label: "Separate aluminium cans from general rubbish", emoji: "🥫", category: "recycle", reason: "Recycling aluminium uses 95% LESS energy than making new aluminium from raw materials. And it can be recycled over and over!" },
  { label: "Eat all the food on your plate — waste nothing", emoji: "🍽️", category: "reduce", reason: "Food waste creates methane gas in landfills — a powerful greenhouse gas. Eating what you take is one of the easiest ways to help the planet!" },
  { label: "Donate old books to a library or school", emoji: "📚", category: "reuse", reason: "Books can be read by dozens of people! Donating means your books go on bringing joy and knowledge instead of sitting in the rubbish bin." },
  { label: "Put plastic bottles in the recycling bin", emoji: "🧴", category: "recycle", reason: "Plastic bottles can be recycled into new bottles, clothing, or even park benches! Always check the recycling symbol on the bottom." },
];
...
export default function ThreeRSortGame() {
      ...
      {!answered ? (
        ...
        <div className="flex flex-col gap-3 pop">
          ...
          <button onClick={handleNext} className="w-full py-4 rounded-2xl font-extrabold text-white text-lg transition-all active:translate-y-0.5" style={{ background: "#1D9E75", boxShadow: "0 4px 0 #116047" }}>
            {current + 1 >= items.length ? "See My Score 🏆" : "Next Action →"}
          </button>
        </div>
      )}
    </div>
