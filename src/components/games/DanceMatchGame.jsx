...
function makeCards() {
  const cards = [];
  PAIRS.forEach((p) => {
    cards.push({ id: `${p.id}-name`, pairKey: p.id, label: `\u{1F483} ${p.name}`, isName: true, flipped: false, matched: false });
    cards.push({ id: `${p.id}-orig`, pairKey: p.id, label: `\u{1F4CD} ${p.origin}`, isName: false, flipped: false, matched: false });
  });
  return shuffle(cards);
}
function DanceMatchGame() {
