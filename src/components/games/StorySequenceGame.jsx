...
export default function StorySequenceGame() {
  ...
  const [placed, setPlaced] = useState<(typeof TALES[0]['steps'][0] | null)[]>([null, null, null, null]);
  ...
  function handleOptionTap(order: number) {
    if (checked) return;
  ...
  function handleSlotTap(idx: number) {
    ...
    if (placed[idx]) {
      ...
      const next = [...placed]; next[idx] = step; setPlaced(next);
      setSelected(null);
    }
  }

  function handleCheck() { if (allFilled) { if (isCorrect) setScore(s => s + 1); setChecked(true); } }

  function handleNext() {
    if (taleIdx + 1 >= TALES.length) { setDone(true); return; }
  ...
  function reset() { setTaleIdx(0); setOptions(shuffle(TALES[0].steps)); setPlaced([null,null,null,null]); setSelected(null); setChecked(false); setScore(0); setDone(false); }

  if (done) return (
    <div className="text-center space-y-5 py-4 font-display">
      <div className="text-7xl animate-bounce">{score === TALES.length ? "🏆" : score >= 2 ? "⭐" : "📖"}</div>
      ...
      <div className="space-y-3 text-left">
        {TALES.map(t => (
          <div key={t.title} className="rounded-2xl p-4" style={{ background: BG, border: `2px solid ${COLOR}` }}>
    ...
    <div className="space-y-3 font-display">
      ...
      <div className="space-y-2">
        {options.map(step => {
          ...
          return (
            <button key={step.order} onClick={() => handleOptionTap(step.order)} disabled={checked}
      ...
      {!checked ? (
        <button onClick={handleCheck} disabled={!allFilled} className="w-full font-extrabold text-lg text-white py-3.5 rounded-2xl transition-all active:translate-y-0.5 disabled:opacity-50" style={{ background: allFilled ? COLOR : "#d1d5db", boxShadow: allFilled ? `0 5px 0 ${DARK}` : "none" }}>Check Story Order ✓</button>
      ) : (
        <div className="space-y-2">
          <div className="rounded-2xl px-5 py-3 text-center" style={{ background: isCorrect ? "#dcfce7" : "#fff7ed", border: `2px solid ${isCorrect ? "#86efac" : "#fdba74"}` }}>
            <p className="font-extrabold text-base" style={{ color: isCorrect ? "#15803d" : "#c2410c" }}>{isCorrect ? "🌟 Perfect!" : "💡 The correct order was 1→2→3→4!"}</p>
          ...
          <button onClick={handleNext} className="w-full font-extrabold text-lg text-white py-3.5 rounded-2xl transition-all active:translate-y-0.5" style={{ background: COLOR, boxShadow: `0 5px 0 ${DARK}` }}>
            {taleIdx + 1 >= TALES.length ? "See My Score 🏆" : "Next Story →"}
          </button>
        </div>
      )}
    </div>
