...
const ITEMS = [
  { label: "Eating fruits and vegetables every day", emoji: "\u{1F966}", category: "healthy", reason: "Fruits and veggies give your body vitamins and minerals to grow strong and fight off sickness!" },
  { label: "Staying up very late every night playing games", emoji: "\u{1F319}", category: "unhealthy", reason: "Children need 9\u201311 hours of sleep a night! Sleep helps your brain grow, your body repair, and your mood stay happy." },
  { label: "Brushing teeth morning and night", emoji: "\u{1F9B7}", category: "healthy", reason: "Brushing twice a day removes food and bacteria that cause cavities. Strong teeth help you eat, smile, and talk!" },
  { label: "Drinking sugary fizzy drinks every day", emoji: "\u{1F964}", category: "unhealthy", reason: "Lots of sugar damages your teeth and gives your body empty calories with no nutrition. Water and milk are much better choices!" },
  { label: "Washing hands before eating and after the toilet", emoji: "\u{1F9FC}", category: "healthy", reason: "Handwashing removes germs before they enter your body \u2014 it's one of the best ways to stop spreading colds and tummy bugs!" },
  { label: "Playing outside and running around every day", emoji: "\u{1F3C3}", category: "healthy", reason: "Exercise makes your heart strong, builds your muscles, and releases happy chemicals in your brain. Aim for at least 60 minutes of active play!" },
  { label: "Eating junk food at every single meal", emoji: "\u{1F354}", category: "unhealthy", reason: "A little junk food is fine as a treat, but eating it all the time deprives your body of the nutrients it needs to grow and learn." },
  { label: "Drinking enough water throughout the day", emoji: "\u{1F4A7}", category: "healthy", reason: "Your body is 60% water! Staying hydrated keeps your brain sharp, your energy high, and your organs working properly." },
  { label: "Sitting still and watching screens for 8 hours", emoji: "\u{1F4F1}", category: "unhealthy", reason: "Too much screen time can strain your eyes, disrupt sleep, and reduce time for exercise and creative play. Breaks every 30 minutes help!" },
  { label: "Eating a healthy breakfast before school", emoji: "\u{1F373}", category: "healthy", reason: "Breakfast fuels your brain for learning! Children who eat breakfast concentrate better, remember more, and feel more energetic all morning." },
  { label: "Sharing drinks and food with a sick friend", emoji: "\u{1F912}", category: "unhealthy", reason: "Sharing cups or cutlery with someone who is sick can spread their germs to you. Always use your own cup and utensils when someone is unwell!" },
  { label: "Going to the doctor for regular check-ups", emoji: "\u{1F3E5}", category: "healthy", reason: "Regular check-ups help doctors catch small problems early, before they become big ones. Prevention is always better than treatment!" }
];
...
function HabitsGame() {
  ...
  if (done) return <div className="flex flex-col items-center gap-5 py-4 text-center font-display">
      <div className="text-7xl animate-bounce">{score === items.length ? "\u{1F3C6}" : score >= 9 ? "\u2B50" : "\u{1F4AA}"}</div>
  ...
  return <div className="flex flex-col gap-4 font-display">
      ...
      {!answered ? <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={handleNext} className="w-full py-4 rounded-2xl font-extrabold text-white text-lg transition-all active:translate-y-0.5" style={{ background: HEALTHY_COLOR, boxShadow: `0 4px 0 ${HEALTHY_DARK}` }}>
            {current + 1 >= items.length ? "See My Score \u{1F3C6}" : "Next Habit \u2192"}
          </button>
        </div>}
    </div>;
}
export {
  HabitsGame as default
};
