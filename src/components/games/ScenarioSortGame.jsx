...
export default function ScenarioSortGame() {
  ...
  if (done) return (
    <div className="text-center space-y-5 py-4 font-display">
      <div className="text-7xl animate-bounce">{score === SCENARIOS.length ? "🏆" : score >= 4 ? "⭐" : "🧠"}</div>
  ...
  return (
    <div className="space-y-4 font-display">
      ...
      {!confirmed ? (
        ...
        <button onClick={handleNext} className="w-full font-extrabold text-xl text-white py-4 rounded-2xl transition-all active:translate-y-0.5" style={{ background: COLOR, boxShadow: `0 5px 0 ${DARK}` }}>{current + 1 >= SCENARIOS.length ? "See My Score 🏆" : "Next Scenario →"}</button>
      )}
    </div>
  );
}
