...
const CATEGORIES = [
  { key: "mammal", label: "Mammal", emoji: "\u{1F418}", color: "#854F0B", bg: "#FFF9E6", desc: "Warm-blooded, has fur/hair, feeds babies milk" },
  { key: "bird", label: "Bird", emoji: "\u{1F985}", color: "#185FA5", bg: "#EBF6FF", desc: "Has feathers and wings, lays eggs" },
  { key: "reptile", label: "Reptile", emoji: "\u{1F98E}", color: "#1D9E75", bg: "#E8FBF5", desc: "Scaly skin, cold-blooded, mostly lays eggs" },
  { key: "fish", label: "Fish", emoji: "\u{1F41F}", color: "#5178c8", bg: "#EDF2FF", desc: "Lives in water, breathes through gills, has fins" },
  { key: "amphibian", label: "Amphibian", emoji: "\u{1F438}", color: "#3B6D11", bg: "#F0FBE4", desc: "Lives in water AND on land; moist skin" }
];
const ANIMALS = [
  { name: "Elephant", emoji: "\u{1F418}", category: "mammal", fact: "Elephants are the largest land animals \u2014 they are warm-blooded and feed their babies milk." },
  { name: "Eagle", emoji: "\u{1F985}", category: "bird", fact: "Eagles are birds of prey with powerful talons. They have feathers and lay eggs in huge nests." },
  { name: "Gecko", emoji: "\u{1F98E}", category: "reptile", fact: "Geckos are small reptiles with scaly skin. They're cold-blooded and can walk on walls!" },
  { name: "Clownfish", emoji: "\u{1F420}", category: "fish", fact: "Clownfish (like Nemo!) live in coral reefs. They breathe through gills and have fins." },
  { name: "Frog", emoji: "\u{1F438}", category: "amphibian", fact: "Frogs start life in water as tadpoles, then grow legs and can live on land \u2014 true amphibians!" },
  { name: "Dolphin", emoji: "\u{1F42C}", category: "mammal", fact: "Even though dolphins live in the sea, they are mammals \u2014 they breathe air and feed their babies milk!" },
  { name: "Penguin", emoji: "\u{1F427}", category: "bird", fact: "Penguins are birds that can't fly, but they are AMAZING swimmers! They have feathers and lay eggs." },
  { name: "Crocodile", emoji: "\u{1F40A}", category: "reptile", fact: "Crocodiles are ancient reptiles \u2014 they've been on Earth for 200 million years! Scaly skin, cold-blooded." },
  { name: "Shark", emoji: "\u{1F988}", category: "fish", fact: "Sharks are powerful fish with rows of sharp teeth. They breathe through gills and never stop swimming!" },
  { name: "Salamander", emoji: "\u{1F98E}", category: "amphibian", fact: "Salamanders look like lizards but are actually amphibians \u2014 they need moist skin and live near water." },
  { name: "Bat", emoji: "\u{1F987}", category: "mammal", fact: "Bats are the only mammals that can truly fly! They are warm-blooded and feed their pups milk." },
  { name: "Parrot", emoji: "\u{1F99C}", category: "bird", fact: "Parrots are intelligent birds with bright feathers. They can mimic sounds and even human words!" }
];
...
function AnimalClassifyGame() {
  ...
  if (done) return <div className="flex flex-col items-center gap-5 py-4 text-center font-display">
      <div className="text-7xl animate-bounce">{score === animals.length ? "\u{1F3C6}" : score >= 9 ? "\u2B50" : "\u{1F43E}"}</div>
  ...
  return <div className="flex flex-col gap-4 font-display">
      ...
      {!answered ? <div className="grid grid-cols-1 gap-2">
  ...
  >
          ...
          <button onClick={handleNext} className="w-full py-4 rounded-2xl font-extrabold text-white text-lg transition-all active:translate-y-0.5" style={{ background: "#1D9E75", boxShadow: "0 4px 0 #116047" }}>
            {current + 1 >= animals.length ? "See My Score \u{1F3C6}" : "Next Animal \u2192"}
          </button>
        </div>}
    </div>;
}
export {
  AnimalClassifyGame as default
};
