...
function PlantGame() {
  ...
  function handleConfirm() {
    ...
    const actualCorrect = part.choices.find((_, i) => i === 0) === selected ? part.choices[0] === part.label || part.choices[0] === "Photosynthesis" : selected === part.label || selected === "Photosynthesis";
  ...
  if (done) return <div className="text-center space-y-5 py-4 font-display">
      <div className="text-7xl animate-bounce">{score === PARTS.length ? "\u{1F3C6}" : score >= 3 ? "\u{1F33F}" : "\u{1F331}"}</div>
      ...
      <div className="rounded-2xl p-4 text-left space-y-2" style={{ background: BG, border: `2px solid ${COLOR}` }}>
        {PARTS.map((p) => <div key={p.key} className="flex items-start gap-2">
  ...
  return <div className="space-y-4 font-display">
    ...
    >
              <span className="text-2xl">{["\u{1F331}", "\u{1FAB5}", "\u{1F343}", "\u{1F338}", "\u2600\uFE0F"][idx] ?? "\u{1F33F}"}</span>
              {choice}
              {confirmed && choice === correctAnswer && <span className="text-green-500">✓</span>}
            </button>;
  })}
      </div>
      {confirmed && <div className="rounded-2xl px-4 py-3 flex gap-3 items-start" style={{ background: isSelectedCorrect ? "#dcfce7" : "#fff7ed", border: `2px solid ${isSelectedCorrect ? "#86efac" : "#fdba74"}` }}>
          <span className="text-2xl">{isSelectedCorrect ? "\u{1F31F}" : "\u{1F4A1}"}</span>
