...
const DARK = "#2f2880";
const BG = "#F0EEFF";
const EMOJIS = ["\u{1F34E}", "\u{1F31F}", "\u{1F438}", "\u{1F98B}", "\u{1F36D}", "\u{1F388}", "\u{1F419}", "\u{1F308}", "\u{1F369}", "\u{1F984}"];
...
function SubtractionGame() {
  ...
  if (done) return <div className="text-center space-y-5 py-4 font-display">
      <div className="text-7xl animate-bounce">{score === TOTAL ? "\u{1F3C6}" : score >= 7 ? "\u2B50" : "\u{1F522}"}</div>
  ...
  return <div className="space-y-4 font-display">
      ...
      <div className="rounded-2xl p-4 text-center" style={{ background: BG, border: `3px solid ${COLOR}` }}>
        <div className="flex flex-wrap justify-center gap-1 mb-3 max-w-[260px] mx-auto">
          {Array.from({ length: q.total }).map((_, i) => <span key={i} className="text-2xl transition-all" style={{ opacity: i >= remaining ? 0.2 : 1, textDecoration: i >= remaining ? "line-through" : "none" }}>{q.emoji}</span>)}
        </div>
    ...
    >
      ...
      {confirmed && <div className="rounded-2xl px-4 py-3 flex gap-2 items-center" style={{ background: selected === q.answer ? "#dcfce7" : "#fff7ed", border: `2px solid ${selected === q.answer ? "#86efac" : "#fdba74"}` }}>
          <span className="text-2xl">{selected === q.answer ? "\u{1F31F}" : "\u{1F4A1}"}</span>
