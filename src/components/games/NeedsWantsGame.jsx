...
const ITEMS = [
  { label: "Food to eat", emoji: "🍚", category: "need", reason: "Our bodies need food every day to have energy, grow, and stay healthy!" },
  { label: "Clean water", emoji: "💧", category: "need", reason: "Water keeps every cell in our body alive. We can only survive a few days without it!" },
  { label: "A home to sleep in", emoji: "🏠", category: "need", reason: "Shelter keeps us safe from rain, cold, and danger — it's one of our most basic needs." },
  { label: "Clothes to wear", emoji: "👕", category: "need", reason: "Clothes protect our bodies from the weather — they're a basic need, though fashion choices are wants!" },
  { label: "A giant teddy bear", emoji: "🧸", category: "want", reason: "Teddy bears are lovely but we don't need them to survive. They're a fun want!" },
  { label: "New video game", emoji: "🎮", category: "want", reason: "Games are great fun but not a necessity — we can live happily without them." },
  { label: "Medicine when sick", emoji: "💊", category: "need", reason: "When we are sick, medicine helps us get better — it becomes a need at that time!" },
  { label: "Sweets and candy", emoji: "🍬", category: "want", reason: "Sweets taste delicious but they're definitely a want — our body doesn't need sugar to survive." },
  { label: "Going to school", emoji: "📚", category: "need", reason: "Education is a basic human right and need — it helps us build our future and understand the world!" },
  { label: "A new bicycle", emoji: "🚲", category: "want", reason: "A bike is a great toy or even transport, but it's a want — we can use other ways to get around." },
  { label: "A doctor's visit", emoji: "🏥", category: "need", reason: "Healthcare keeps us healthy and alive — it's an important need for everyone." },
  { label: "Ice cream every day", emoji: "🍦", category: "want", reason: "Ice cream is yummy, but eating it every day would actually harm our health — it's definitely a want!" },
];
...
export default function NeedsWantsGame() {
  ...
  const [answered, setAnswered] = useState<"need" | "want" | null>(null);
  ...
  function handleAnswer(choice: "need" | "want") {
    if (answered) return;
  ...
  if (done) {
    ...
    return (
      <div className="flex flex-col items-center gap-5 py-4 text-center font-display">
        <div className="text-7xl animate-bounce">{perfect ? "🏆" : score >= 9 ? "⭐" : "💰"}</div>
      ...
      {!answered ? (
        ...
        <div className="flex flex-col gap-3 pop">
          ...
          <button onClick={handleNext} className="w-full py-4 rounded-2xl font-extrabold text-white text-lg transition-all active:translate-y-0.5" style={{ background: NEED_COLOR, boxShadow: `0 4px 0 ${NEED_DARK}` }}>
            {current + 1 >= items.length ? "See My Score 🏆" : "Next Item →"}
          </button>
        </div>
      )}
    </div>
