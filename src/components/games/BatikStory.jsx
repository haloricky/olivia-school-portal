...
const CHAPTERS = [
  ...
  {
    id: "patterns",
    title: "Famous Patterns \u2014 Each Tells a Story",
    emoji: "\u{1F33F}",
    kind: "svgpattern",
    body: "Different regions of Indonesia have their own unique batik patterns! Batik from Yogyakarta (Jogja) often uses brown and cream colours with geometric shapes \u2014 it's classic and dignified. Solo batik tends to be more delicate and fine. Pekalongan batik has bright rainbow colours influenced by Chinese and Dutch traders. Cirebon batik features sea creatures and clouds \u2014 because it's a coastal city!",
    fact: "The most sacred batik pattern is called Parang (meaning 'sword' in Javanese). In the royal court of Yogyakarta, only the king and royal family were allowed to wear it!"
  },
  {
    id: "regions",
    title: "Batik Across Indonesia",
    emoji: "\u{1F5FA}\uFE0F",
    kind: "svgmap",
    body: "Batik is made all across the Indonesian archipelago \u2014 not just in Java! Batik Madura has bold red, blue, and black colours. Batik Bali often features Balinese gods and nature scenes. Batik Kalimantan uses dayak tribal patterns. Even Papua has its own batik inspired by traditional carvings! Each island's batik reflects its people's stories, beliefs, and environment.",
  ...
  {
    id: "unesco",
    title: "Recognised by the World \u2014 UNESCO 2009",
    emoji: "\u{1F3C6}",
    kind: "svgunesco",
    body: "In 2009, UNESCO \u2014 the United Nations organisation that protects the world's most important cultures \u2014 officially recognised Indonesian batik as an 'Intangible Cultural Heritage of Humanity'. This is like winning the gold medal of world culture! It means the whole world agrees that batik is precious and must be protected. Indonesia is very proud of this recognition.",
    fact: "When UNESCO announced the recognition on 2 October 2009, Indonesia declared that date 'National Batik Day'. Now every 2nd October, Indonesians wear batik to school, work, and everywhere!"
  },
  {
    id: "design",
    title: "Design Your Own Batik!",
    emoji: "\u2728",
    kind: "interactive",
...
function ImgCard({ src, alt, caption }) {
  ...
  if (errored) return <div
    className="w-full rounded-2xl flex items-center justify-center text-gray-400 text-sm font-bold"
    style={{ height: 280, background: "#f3f4f6", border: "2px dashed #d1d5db" }}
  >
      📷 Image unavailable
    </div>;
  return <div className="relative w-full rounded-2xl overflow-hidden" style={{ height: 280 }}>
    ...
    style={{ height: 280, opacity: loaded ? 1 : 0 }}
...
function PatternScene() {
  return <svg viewBox="0 0 320 200" className="w-full rounded-2xl" style={{ height: 200 }}>
    ...
    /* Jogja pattern - left */
      ...
      {[0, 1, 2, 3, 4, 5, 6, 7].map((i) => <g key={i}>
          <path d={`M ${15 + i % 2 * 10} ${30 + i * 18} L ${90} ${30 + i * 18}`} stroke="#8B1A4A" strokeWidth="1.5" />
          <circle cx={20 + i % 3 * 20} cy={38 + i * 18} r={5} fill="none" stroke="#8B1A4A" strokeWidth="1" />
        </g>)}
    ...
    /* Pekalongan - middle */
      ...
      {["\u{1F33A}", "\u{1F98B}", "\u{1F338}", "\u{1F426}", "\u{1F33A}", "\u{1F98B}", "\u{1F338}", "\u{1F426}"].map((e, i) => <text key={i} x={120 + i % 2 * 40} y={45 + Math.floor(i / 2) * 38} fontSize="22">{e}</text>)}
    ...
    /* Cirebon - right */
      ...
      {[0, 1, 2].map((i) => <g key={i}>
          <circle cx={265} cy={60 + i * 50} r={22} fill="none" stroke="#4a90c0" strokeWidth="2" />
          <circle cx={265} cy={60 + i * 50} r={12} fill="#4a90c0" opacity="0.3" />
          <text x={265} y={64 + i * 50} textAnchor="middle" fontSize="14">☁️</text>
        </g>)}
...
function MapScene() {
  return <svg viewBox="0 0 320 200" className="w-full rounded-2xl" style={{ height: 200 }}>
      ...
      {[
    { label: "Aceh", x: 30, y: 80, color: "#FF85A1" },
    { label: "Batik Jogja", x: 145, y: 120, color: "#8B1A4A" },
    { label: "Pekalongan", x: 130, y: 100, color: "#FFB347" },
    { label: "Cirebon", x: 148, y: 110, color: "#4a90c0" },
    { label: "Madura", x: 180, y: 105, color: "#e03030" },
    { label: "Bali", x: 200, y: 120, color: "#FFD700" },
    { label: "Kalimantan", x: 210, y: 75, color: "#4CAF50" }
  ].map((p) => <g key={p.label}>
...
function UnescoScene() {
  return <svg viewBox="0 0 320 200" className="w-full rounded-2xl" style={{ height: 200 }}>
      ...
      <text x="160" y="155" textAnchor="middle" fontSize="9" fill="#8B1A4A" fontWeight="bold" fontFamily="Nunito">Indonesian Batik recognised by the World!</text>
    </svg>;
}
function InteractiveBatik() {
  ...
  >
    ...
    style={{
      ...
      height: 28,
      backgroundColor: c,
      border: activeColor === c ? "3px solid white" : "2px solid #ddd",
      boxShadow: activeColor === c ? `0 0 0 3px ${c}` : "none"
    }}
  >
          </button>)}
        <button
    onClick={() => setColors(Array(GRID * GRID).fill("#f5e6cc"))}
    className="px-2 py-1 rounded-lg text-xs font-bold text-gray-500 bg-gray-100"
  >
          Clear
        </button>
      </div>
...
function BatikStory() {
  ...
  const accent = "#8B1A4A";
  const accentDark = "#5a0e2e";
  return <div className="flex flex-col gap-3" style={{ fontFamily: "Nunito, sans-serif" }}>
      <div className="text-center">
        <h3 className="text-lg font-extrabold mb-0.5" style={{ color: accent }}>
          Batik — Indonesia's Living Art
        </h3>
