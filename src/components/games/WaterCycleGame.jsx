...
const STEPS = [
  ...
  { key: "precipitation", label: "Precipitation", emoji: "🌧️", color: "#4a90d9", desc: "When clouds become too heavy with water, the droplets fall back to Earth as rain, snow, hail, or sleet." },
  { key: "collection",    label: "Collection",    emoji: "🌊", color: "#1D9E75", desc: "Rain water flows into rivers, lakes, and oceans — and soaks into the ground. Then the sun heats it up again and the cycle starts over!" },
];
...
export default function WaterCycleGame() {
  ...
  const [placed, setPlaced] = useState<(typeof STEPS[0] | null)[]>([null, null, null, null]);
  ...
  function handleOptionTap(key: string) {
    ...
    if (usedKeys.includes(key)) {
      ...
      const newPlaced = [...placed]; newPlaced[idx] = null; setPlaced(newPlaced);
      setSelected(null);
    } else {
      setSelected(key);
    }
  ...
  function handleSlotTap(idx: number) {
    ...
    if (placed[idx]) {
      ...
      const newPlaced = [...placed]; newPlaced[idx] = step; setPlaced(newPlaced);
      setSelected(null);
    }
  }
  ...
  function handleReveal() { setDone(true); }
  function reset() { setPlaced([null,null,null,null]); setSelected(null); setChecked(false); setDone(false); }
    ...
    <div className="space-y-4 font-display">
      ...
      <div className="grid grid-cols-2 gap-2">
        {options.map(step => {
          ...
          return (
            <button key={step.key} onClick={() => handleOptionTap(step.key)} disabled={checked}
      ...
      {!checked && (
        <button onClick={handleCheck} disabled={!allFilled} className="w-full font-extrabold text-xl text-white py-4 rounded-2xl transition-all active:translate-y-0.5 disabled:opacity-50" style={{ background: allFilled ? COLOR : "#d1d5db", boxShadow: allFilled ? `0 5px 0 ${DARK}` : "none" }}>Check My Order ✓</button>
      )}
      ...
      {done && (
        <div className="space-y-3">
          {STEPS.map((step, i) => (
            <div key={step.key} className="rounded-2xl p-3 flex items-start gap-3" style={{ background: step.color + "18", border: `2px solid ${step.color}` }}>
