...
function InstrumentsGame() {
  ...
  if (done) return <div className="flex flex-col items-center gap-5 py-4 text-center font-display">
      ...
      <div className="space-y-3 w-full text-left">
        {INSTRUMENTS.map((inst) => <div key={inst.id} className="rounded-2xl p-4 flex items-start gap-3" style={{ background: BG, border: `2px solid ${inst.color}` }}>
  ...
  return <div className="space-y-4 font-display">
      ...
      {matched > 0 && <div className="flex gap-2 flex-wrap">
          {INSTRUMENTS.filter((inst) => cards.find((c) => c.pairKey === inst.id && c.matched)).map((inst) => <div key={inst.id} className="rounded-xl px-2.5 py-1 text-xs font-extrabold flex items-center gap-1" style={{ background: BG, color: inst.color, border: `1.5px solid ${inst.color}` }}>
    ...
    >
              {isUp ? <>
                  <span className="text-2xl">{card.isName ? inst.emoji : "\u{1F4CD}"}</span>
