...
const BG = "#FFF0F5";
const PAIRS = [
  { right: "Right to Education \u2014 every child deserves to go to school and learn", duty: "Duty to study hard and help classmates learn too", emoji: "\u{1F4DA}", color: "#1D9E75" },
  { right: "Right to Safety \u2014 every person deserves to feel safe from harm", duty: "Duty to follow rules and treat others with kindness", emoji: "\u{1F6E1}\uFE0F", color: "#185FA5" },
  { right: "Right to Clean Water \u2014 every person needs safe water to drink", duty: "Duty to save water and not waste or pollute it", emoji: "\u{1F4A7}", color: "#5178c8" },
  { right: "Right to Free Speech \u2014 you can share your thoughts and ideas", duty: "Duty to listen respectfully when others share theirs", emoji: "\u{1F5E3}\uFE0F", color: "#854F0B" },
  { right: "Right to Healthcare \u2014 every person deserves medical care when sick", duty: "Duty to look after your health and not spread illness", emoji: "\u{1F3E5}", color: "#993556" },
  { right: "Right to a Clean Environment \u2014 everyone deserves clean air and nature", duty: "Duty to reduce rubbish, recycle, and protect our Earth", emoji: "\u{1F33F}", color: "#3B6D11" }
];
const QUESTIONS = [
...
function RightsMatchGame() {
  ...
  if (done) return <div className="text-center space-y-5 py-4 font-display">
      ...
      <div className="space-y-2 w-full text-left">
        {PAIRS.map((p, i) => <div key={i} className="rounded-xl p-3" style={{ background: p.color + "15", border: `1.5px solid ${p.color}` }}>
