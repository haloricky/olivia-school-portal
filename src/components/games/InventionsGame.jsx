...
const PAIRS = [
  { id: "paper",     name: "📜 Paper",      desc: "Used to write books, draw pictures, and send letters", emoji: "📜" },
  { id: "compass",   name: "🧭 Compass",    desc: "Helps sailors find North and navigate the oceans",     emoji: "🧭" },
  { id: "printing",  name: "🖨️ Printing",   desc: "Copies words onto pages so many people can read them", emoji: "🖨️" },
  { id: "fireworks", name: "🎆 Fireworks",  desc: "Beautiful explosions of colour used in celebrations",  emoji: "🎆" },
  { id: "silk",      name: "🧵 Silk",       desc: "Smooth, shiny fabric made by silkworms — very precious", emoji: "🧵" },
  { id: "gunpowder", name: "💥 Gunpowder",  desc: "Black powder that explodes — led to fireworks first!", emoji: "💥" },
];
...
export default function InventionsGame() {
  ...
  if (done) return (
    <div className="flex flex-col items-center gap-5 py-4 text-center font-display">
      <div className="text-7xl animate-bounce">{moves <= 7 ? "🏆" : "⭐"}</div>
      ...
      <div className="grid grid-cols-2 gap-2 w-full text-left">
        {PAIRS.map(p => (
          <div key={p.id} className="rounded-xl px-3 py-2" style={{ background: BG, border: `1.5px solid ${COLOR}` }}>
        ...
        {cards.map(card => {
          ...
          return (
            <button key={card.id} onClick={() => handleFlip(card.id)} disabled={isUp || locked}
              ...
              {isUp ? (
                <span className="text-xs font-extrabold leading-tight" style={{ color: card.matched ? "#15803d" : COLOR }}>{card.label}</span>
              ) : (
