...
const CHAPTERS = [
  { id: 'why', title: 'What Are Rules?', emoji: '📋', scene: 'why',
  ...
  { id: 'home', title: 'Rules at Home', emoji: '🏠', scene: 'home',
    body: "Every home has its own rules, and they're there to keep the family happy and healthy! Things like brushing your teeth before bed, tidying your room, and going to sleep on time. When everyone in the family follows the rules, home feels warm and peaceful.",
  ...
  { id: 'school', title: 'Rules at School', emoji: '🏫', scene: 'school',
    body: "At school, rules help everyone learn! We listen to the teacher so everyone can hear. We don't hit or push because everyone deserves to feel safe. We take turns so everyone gets a chance. We raise our hand before speaking so the classroom isn't noisy.",
  ...
  { id: 'road', title: 'Rules on the Road', emoji: '🚦', scene: 'road',
    body: "Road rules keep us safe every single day! Red means STOP, green means GO, and yellow means slow down. Pedestrians use the zebra crossing to cross safely. Everyone must wear a seatbelt. Without these rules, roads would be very dangerous places.",
  ...
  { id: 'fair', title: 'Rules Make Things Fair', emoji: '⚖️', scene: 'fair',
    ...
    fact: "The word 'fair' comes from the Old English word 'fæger', which means beautiful! Being fair really is beautiful." },
  { id: 'outrules', title: 'Making Our Own Rules!', emoji: '✍️', scene: 'our',
    body: "Rules don't have to come only from grown-ups! You can help make rules too. As a family, you can sit together and decide: what are the most important rules for our home? Rules that we make together feel more special — because we all chose them!",
...
function SceneIllustration({ scene }) {
  const scenes = {
    why: (
      <svg viewBox="0 0 320 180" className="w-full rounded-2xl">
...
export default function RulesStory() {
  const [page, setPage] = useState(0)
  const chapter = CHAPTERS[page]
  const total = CHAPTERS.length
  const accent = '#4CAF50'; const accentDark = '#2e7d32'

  return (
    <div className="flex flex-col gap-3 font-display">
      <div className="text-center">
        <h3 className="text-lg font-extrabold mb-0.5" style={{ color: '#2e7d32' }}>Rules — A Picture Story</h3>
