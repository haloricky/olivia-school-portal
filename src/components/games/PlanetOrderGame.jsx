...
const PLANETS = [
  ...
  { key: "venus", name: "Venus", emoji: "\u{1F7E1}", color: "#FFD700", desc: "The hottest planet \u2014 covered in thick clouds that trap heat like a giant greenhouse" },
  { key: "earth", name: "Earth", emoji: "\u{1F30D}", color: "#1D9E75", desc: "Our home! The only known planet with liquid water, air to breathe, and life" },
  { key: "mars", name: "Mars", emoji: "\u{1F534}", color: "#E53935", desc: "The Red Planet \u2014 covered in red dust and iron oxide. Scientists are planning to send humans here!" },
  { key: "jupiter", name: "Jupiter", emoji: "\u{1F7E0}", color: "#FF8C00", desc: "The largest planet \u2014 so big that 1,300 Earths could fit inside it! Has a giant storm called the Great Red Spot" },
  { key: "saturn", name: "Saturn", emoji: "\u{1FA90}", color: "#C4A35A", desc: "Famous for its beautiful rings made of ice and rock. Saturn is so light it would float on water!" },
  { key: "uranus", name: "Uranus", emoji: "\u{1F535}", color: "#5aa0bd", desc: "An ice giant that spins on its side! It takes 84 Earth years to orbit the Sun once" },
  { key: "neptune", name: "Neptune", emoji: "\u{1F499}", color: "#3F51B5", desc: "The furthest planet \u2014 so far that sunlight takes 4 hours to reach it. Winds reach 2,100 km/h!" }
];
...
function PlanetOrderGame() {
  ...
  const [placed, setPlaced] = useState(Array(8).fill(null));
  ...
  return <div className="space-y-3 font-display">
    ...
    return <button
      key={planet.key}
      onClick={() => handleOptionTap(planet.key)}
      disabled={checked}
    ...
    >
      ...
      {!checked && <button onClick={handleCheck} disabled={!allFilled} className="w-full font-extrabold text-lg text-white py-3.5 rounded-2xl transition-all active:translate-y-0.5 disabled:opacity-50" style={{ background: allFilled ? COLOR : "#d1d5db", boxShadow: allFilled ? `0 5px 0 ${DARK}` : "none" }}>Check My Order ✓</button>}

      {checked && !done && <div className="space-y-2">
      ...
      {done && <div className="space-y-2">
          {PLANETS.map((p, i) => <div key={p.key} className="rounded-2xl p-3 flex items-start gap-3" style={{ background: p.color + "18", border: `2px solid ${p.color}` }}>
