  ...
  return (
    <div className="cursor-pointer select-none" style={{ perspective: '600px', height: '120px' }} onClick={() => setFlipped(f => !f)}>
      <div className="relative w-full h-full transition-transform duration-500" style={{ transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
        <div className="absolute inset-0 rounded-2xl flex flex-col items-center justify-center gap-1 shadow-md" style={{ backfaceVisibility: 'hidden', backgroundColor: c.bg, boxShadow: `0 4px 0 ${c.dark}` }}>
        ...
        <div className="absolute inset-0 rounded-2xl flex flex-col items-center justify-center gap-1 shadow-md" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', backgroundColor: '#fff', border: `3px solid ${c.bg}`, boxShadow: `0 4px 0 ${c.dark}` }}>
...
function ComingSoonGame({ gameType }) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
      ...
      <p className="text-gray-500 text-sm font-display">The <span className="font-bold text-primary">"{gameType}"</span> game is being built. Stay tuned, Olivia!</p>
  ...
  if (!content || !lesson) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 font-display">
        ...
        <button onClick={() => navigate('/')} className="press-btn px-6 py-3 rounded-chunky font-display font-bold text-white bg-primary">Back to Home</button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-cream font-display">
      <div className="px-4 pt-6 pb-4" style={{ backgroundColor: subject?.bg ?? '#FFF5F8' }}>
        <button onClick={() => navigate('/')} className="flex items-center gap-2 text-sm font-bold mb-4 rounded-xl px-3 py-1.5 transition-all active:scale-95" style={{ color: subject?.color ?? '#FF85A1', backgroundColor: 'rgba(255,255,255,0.6)' }}>
          ← Back
        </button>
        <div className="flex items-start gap-3">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl flex-shrink-0" style={{ backgroundColor: (subject?.color ?? '#FF85A1') + '22' }}>{subject?.emoji ?? '📚'}</div>
          <div>
            <div className="text-xs font-extrabold uppercase tracking-widest mb-0.5" style={{ color: subject?.color ?? '#FF85A1' }}>Week {week} · {subject?.name ?? 'Lesson'}</div>
        ...
        <div className="flex gap-2 mt-5 overflow-x-auto pb-1">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-2xl font-extrabold text-sm whitespace-nowrap transition-all active:translate-y-0.5 flex-shrink-0 ${activeTab === tab.key ? 'text-white shadow-md' : 'text-gray-500 bg-white/70 hover:bg-white'}`}
      ...
      <div className="px-4 py-5 max-w-2xl mx-auto">
        {activeTab === 'story' && (
          <div className="flex flex-col gap-4">
            <div className="rounded-3xl p-5 bg-white shadow-sm border border-pink-100">
              <div className="flex items-center gap-2 mb-3"><span className="text-2xl">📖</span><span className="text-sm font-extrabold text-primary uppercase tracking-wider">Opening Story</span></div>
              <p className="text-gray-700 text-base leading-relaxed font-semibold italic">"{content.story}"</p>
            </div>
            {content.game_type === 'continents' && (
              <div className="rounded-3xl p-4 bg-white shadow-sm border border-blue-100"><WorldMapDisplay /></div>
            )}
            <div className="flex items-center gap-2 mt-1"><span className="text-xl">💡</span><h3 className="font-extrabold text-gray-700 text-base">What We Learn</h3></div>
            <div className="flex flex-col gap-3">
              {content.teaching.map((point, i) => (
                <div key={i} className="flex items-start gap-3 rounded-2xl px-4 py-3 bg-white shadow-sm border border-pink-50">
                  <div className="w-7 h-7 rounded-xl flex items-center justify-center text-white text-sm font-extrabold flex-shrink-0 mt-0.5" style={{ backgroundColor: subject?.color ?? '#FF85A1' }}>{i + 1}</div>
            ...
            <div className="rounded-3xl p-5 mt-2 border-2" style={{ backgroundColor: '#FFF9E6', borderColor: '#FFD700' }}>
              <div className="flex items-center gap-2 mb-2"><span className="text-2xl">✏️</span><span className="text-sm font-extrabold text-yellow-600 uppercase tracking-wider">Today's Activity</span></div>
        ...
        {activeTab === 'learn' && (
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">{content.vocab.map((v, i) => <FlipCard key={v.en} en={v.en} id={v.id} index={i} />)}</div>
        ...
        {activeTab === 'play' && (
          <div className="rounded-3xl bg-white shadow-sm border border-pink-100 p-4">
            {content.game_type === 'continents' ? <WorldMapGame />
              ...
              : content.game_type === 'body' ? <BodyGame />
