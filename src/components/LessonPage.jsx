...
export default function LessonPage() {
  ...
  if (!content || !lesson) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 font-display">
        <div className="text-6xl">📚</div>
        <h2 className="text-2xl font-extrabold" style={{ color: '#FF85A1' }}>Lesson not found!</h2>
        <button
          onClick={() => navigate('/')}
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#FFF5F8', fontFamily: 'Nunito, sans-serif' }}>
          ...
          {tabs.map(tab => (
            <button
              ...
              style={
                activeTab === tab.key
                  ? {
                      backgroundColor: subject?.color ?? '#FF85A1',
                      color: '#fff',
                      boxShadow: `0 3px 0 ${subject?.color ?? '#FF85A1'}99`,
                    }
                  : { backgroundColor: 'rgba(255,255,255,0.7)', color: '#6b7280' }
              }
            >
              <span>{tab.emoji}</span>
              <span>{tab.label}</span>
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-3xl p-5 bg-white shadow-sm border border-pink-100">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-extrabold uppercase tracking-wider" style={{ color: '#FF85A1' }}>
                  Opening Story
                </span>
              </div>
            ...
            <div
              className="rounded-3xl p-5 mt-2 border-2"
              style={{ backgroundColor: '#FFF9E6', borderColor: '#FFD700' }}
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">✏️</span>
                <span className="text-sm font-extrabold text-yellow-600 uppercase tracking-wider">
                  Today's Activity
                </span>
              </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="text-center mb-2">
              ...
              <p className="text-sm text-gray-400 font-semibold">
                Tap each card to see it in Bahasa Indonesia!
              </p>
            </div>
        ...
        {activeTab === 'print' && (
          <div className="flex flex-col items-center gap-6 py-6">
            ...
            <a
              ...
              className="flex items-center gap-3 px-8 py-4 rounded-2xl font-extrabold text-white text-lg"
              style={{ backgroundColor: '#FF85A1', boxShadow: '0 4px 0 #cc5a74' }}
            >
              <span>📥</span> Download Worksheet
            </a>
            <div
              className="rounded-2xl px-5 py-4 max-w-xs text-center border"
              style={{ backgroundColor: '#FFF5F8', borderColor: '#ffd6e0' }}
            >
              <p className="text-xs font-bold mb-1" style={{ color: '#FF85A1' }}>TODAY'S ACTIVITY</p>
